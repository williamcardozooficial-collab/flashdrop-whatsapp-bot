...
app.post('/api/send-group-message', async (req, res) => {
  ...
  try {
    ...
    let finalMessage = message; let mentionIds = []; if (mentionAll) { let lastMentionErr = null; for (let attempt = 1; attempt <= 3 && mentionIds.length === 0; attempt++) { try { const chats = await client.getChats(); const chat = chats.find(c => c.id && c.id._serialized === _groupLink.trim()); let participants = (chat && chat.participants && chat.participants.length) ? chat.participants : []; mentionIds = participants.map(p => { if (!p || !p.id) return null; if (p.id._serialized) return p.id._serialized; if (typeof p.id === 'string') return p.id; if (p.id.user) return p.id.user + '@c.us'; return null; }).filter(Boolean); lastMentionErr = null; } catch (eMention) { lastMentionErr = eMention; if (attempt < 3) { await new Promise(r => setTimeout(r, 1500)); } } } if (mentionIds.length) { finalMessage = finalMessage + ' ' + mentionIds.map(id => '@' + id.split('@')[0]).join(' '); } if (lastMentionErr) { logger.log('error', 'Erro ao buscar participantes: ' + (lastMentionErr && (lastMentionErr.stack || lastMentionErr.message) ? String(lastMentionErr.stack || lastMentionErr.message) : String(lastMentionErr))); restartClient(); } } await client.sendMessage(_groupLink.trim(), finalMessage, mentionIds.length ? { mentions: mentionIds } : undefined);
