...
app.post('/api/send-group-message', async (req, res) => {
  ...
  try {
    ...
    let finalMessage = message; let mentionIds = []; if (mentionAll) { try { const chats = await client.getChats(); const chat = chats.find(c => c.id && c.id._serialized === _groupLink.trim()); let participants = (chat && chat.participants && chat.participants.length) ? chat.participants : []; mentionIds = participants.map(p => { if (!p || !p.id) return null; if (p.id._serialized) return p.id._serialized; if (typeof p.id === 'string') return p.id; if (p.id.user) return p.id.user + '@c.us'; return null; }).filter(Boolean); if (mentionIds.length) { finalMessage = finalMessage + ' ' + mentionIds.map(id => '@' + id.split('@')[0]).join(' '); } } catch (eMention) { logger.log('error', 'Erro ao buscar participantes: ' + (eMention && (eMention.stack || eMention.message) ? String(eMention.stack || eMention.message) : String(eMention))); } } await client.sendMessage(_groupLink.trim(), finalMessage, mentionIds.length ? { mentions: mentionIds } : undefined);
